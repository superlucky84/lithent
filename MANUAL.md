...
Lithent는 가상돔을 사용하는데 최소로 꼭 필요한 **기본기능**과, 기본기능을 응용하여 조합한 **확장기능**으로 나누어 개발하였습니다. 
...
콜백이나 디팬던시에 `props`를 사용할 때는 `props.` 프리픽스를 전부 다 붙여서 사용해야 정확히 비교됩니다. 왜냐하면 `마운터`는 컴포넌트가 마운트 될 때 한 번만 실행되고 그 안에서 정의된 콜백함수들이나 def체크 함수는 컴포넌트가 업데이트될 때마다 클로저를 이용해 `props`값에 접근하기 때문입니다. 앞서 업데이터에서 설명한 이슈와 같습니다.
