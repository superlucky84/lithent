...
|---|---:|---:|---:|---|
| **preact 10.29.8 `preact.min.js`** (코어, hook 미포함) | 11,322 | 4,841 | **4,402** | 2026-08-20 재측정 (unpkg) |
| preact 10.29.8 `preact/hooks` | 3,663 | 1,513 | 1,388 | 〃 |
| preact 10.29.8 `preact.module.js` (ESM) | 11,693 | 4,846 | 4,416 | 〃 |

**⚠️ 이 비교는 동일 기능 기준이 아니다 (A10).** preact 코어는 hook을 포함하지 않는 반면
lithent 코어는 라이프사이클 hook을 포함한다. RC-1의 목표(4,402B 이하)는 이 차이를 반영하지 않고
설정되었다 — 상세는 A10 참조.

preact는 ESM 산출물(4,416B br)도 min과 거의 같은 크기다. 다만 이것이 ESM 압축의 실효성을
입증하지는 않는다 — §3.1 참조.
...
- **A10. preact와의 코어 대 코어 비교는 동일 기능 기준이 아니다 (2026-08-20).**
  두 라이브러리가 패키지 경계를 다른 위치에 긋는다.

  | | preact | lithent |
  |---|---|---|
  | 라이프사이클 (effect/ref) | `preact/hooks` (별도) | **코어 포함** |
  | 상태 (state/computed/store) | `preact/hooks` (별도) | `lithent/helper` (별도) |

  실측 (brotli):

  | 구성 | preact | lithent | 차이 |
  |---|---:|---:|---:|
  | 코어끼리 (표면적) | 4,402 | 4,734 | +332 |
  | 코어 + 상태관리 | 5,790 | 6,370 | +580 |

  즉 코어끼리 비교는 lithent에 불리하고(hook 포함분 손해),
  전체 스택 비교는 여전히 preact가 작다. **어느 쪽도 깔끔한 승부가 아니다.**

  메타파일상 lithent 코어의 hook 모듈은 1,331B(minified, 11.5%)이며,
  이를 제외한 순수 VDOM은 약 4,190B br로 **추정**된다(압축이 선형이 아니므로 추정치).
  다만 A7에서 확인했듯 이 hook들은 `wDom.ts`/`render.ts`가 내부적으로 참조하므로
  트리셰이킹으로 제거되지 않는다 — 즉 **분리 가능한 선택 기능이 아니다.**

  **RC-1에 대한 함의**: "preact 이하(4,402B)"라는 목표는 hook 포함 산출물을
  hook 미포함 산출물과 맞붙인 것이어서 **전제가 잘못 설정되었다.**
  RC-10(G2 중단)의 근거를 보강한다 — 성능 트레이드 문제 이전에 목표 자체가 부정확했다.
...
- [x] ~~**RC-1**~~ *(전제 오류 확인, A10 참조)*: 크기 목표 — **빌드 수정 + preact 이하(brotli ≤ 4,410B)**.
  **전제가 잘못되었다** — preact 코어는 hook 미포함, lithent 코어는 hook 포함이라 동일 기준이 아니다.
  목표 수치는 RC-10(중단)으로 실효되었으며, 재개 시 기준을 다시 정의해야 한다.
...
- [x] **RC-10**: G2(코드 축소) 착수 여부 — **중단**. G1·G4까지만 수행하고 마무리한다.
  근거 1: G2의 목표(-333B)는 `feat/alterSpeed`가 30배 성능 개선을 위해 지불한 +565B(br) 중
  59%를 되사오는 것에 해당한다(§4 G2, `../performance-improvement/CODE_WALKTHROUGH.md` §6).
  근거 2 (A10): 목표 수치 자체가 hook 포함/미포함을 혼동해 설정되었다.
...
| 항목 | 상태 | 재개 조건 |
|---|---|---|
| **G2** 코드 축소 (-333B) | RC-10으로 중단 | 재개하려면 **목표부터 다시 정의**해야 한다(A10 — 기존 수치는 hook 포함/미포함 혼동). 착수 시 **벤치 기준선 선행 고정**과 속도 회귀 시 롤백 규칙이 전제 |
