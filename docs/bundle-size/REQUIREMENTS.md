...
- 상태: **G1·G4 완료 — G2(코드 축소) 및 G5(회귀 방지) 대기**
...
- **done**: 문제 실측(P1~P3), 기준선 확보(§2), 모듈별 기여도 분석, RC-1~RC-4 결정,
  vite 원인 규명 및 5~8 버전별 조사(A5), PURE 주석 영향 검증(A6), 트리셰이킹 여지 측정(A7),
  빌드 훅 경로 4종 검증 및 `writeBundle` 해법 확정(A8, 소스맵 역추적 정확도 포함), preact 재측정(§2.4),
  PURE/공백 양립 불가 확인 및 생태계 표본 조사(A9), 가치 재평가(§3) 및 RC-4 개정,
  **G1 구현 완료 및 전체 검증 통과(§10) — 7개 아티팩트 -2,049B gzip(-15.9%), 소스맵 정합 유지**,
  **G4 문서 정정 완료(§11) — UMD 기준으로 고정해 G1 반영 여부와 무관하게 안정**.
- **next**:
  1. **G2(코드 축소)** — UMD 기준 brotli 4,735B → 4,402B, **-333B 필요**.
     `render.ts`(42.7%) 분석부터. 이번 작업에서 유일하게 남은 실질 과제이며 비용이 크다.
  2. **G5(회귀 방지)** — RC-6 결정 후 크기 예산 검사 수단 추가.
  3. main 반영 여부 판단 — 브랜치 실물을 보고 결정 (RC-8 단서).
  4. (선택) `README.md`/`MANUAL.md` prettier 정리 — 기존부터 미준수. 별도 작업 권장.
- **blockers**:
  - ~~S2 근본 원인 미규명~~ → **해소** (A5, 2026-08-20).
  - ~~PURE 주석 손실 리스크~~ → **부분 해소** (A6: lithent 기준 0~3B). 단 범용 도구화는 부적합(A9).
  - ~~A4(preact 재측정) 미수행~~ → **해소** (preact 10.29.8 = 4,402B br, 2026-08-20).
  - ~~D1(수정 수단) 미확정~~ → **해소** (A8, `writeBundle` 훅 검증 완료, 2026-08-20).
  - ~~RC-8(G1 채택 여부) 미결~~ → **해소** — (a) 채택, 구현 완료 (§10).
  - ~~A1(전체 테스트 동등성 검증) 미수행~~ → **해소** — 258개 테스트 전체 통과 (§10.3).
  - A9의 bundlephobia 측정 방식 미검증 — G1의 잔여 명분에 영향. **남아 있음.**
  - RC-6(회귀 방지 장치 형태) 미결 — G5 착수 전 필요.
  - ~~DC-1(소스맵 처리) 미결~~ → **해소** (A8, `writeBundle` 방식 검증 완료, 2026-08-20).
- **commit**: 기준 `f3921cc` → 브랜치 `feat/bundleSize` (아래 커밋 참조)
