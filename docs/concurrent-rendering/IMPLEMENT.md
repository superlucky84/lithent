...
- [ ] 8-4. `workLoop` + `performUnitOfWork` 구현, `shouldYield()` 연동
      — **형제 사이에서 검사할 것.** 깊이 방향에만 넣으면 §Phase 7 표가 움직이지 않는다
      (400단 트리 0.2ms vs 형제 10k 18~60ms)
...
- [ ] 8-10. bench 재측정 — `pnpm bench:units`를 §Phase 7과 **같은 표**로 비교.
      DC-6 판정 기준(대규모 시나리오 총 체감) 적용
- [ ] 8-11. 크기 실측
...
- next: **Phase 8 — 파이버 자료구조 + work loop (D7, D10)**. 진입 승인됨(7-5).
  - **§Phase 8 진입부의 경고를 먼저 읽을 것** — T2는 8+9+10 전부다. Phase 9 전에
    "중단 가능"을 주장하지 말 것 (REQUIREMENTS §2.1).
  - **8-4의 `shouldYield()`는 형제 사이에 넣어야 한다.** 깊이 방향에만 넣으면
    §Phase 7 표가 하나도 움직이지 않는다 — 비싼 단위는 깊은 트리가 아니라
    형제 10,000개를 훑는 모양이다.
  - 진행 후 `pnpm bench:units`를 다시 돌려 같은 표로 비교한다 (8-10의 DC-6 판정).
