...
> _(`pnpm bench:units` 결과를 여기 붙인다. 16ms 초과 단위가 하나라도 있으면 7-5,
> 없으면 7-4다.)_
...
- done:
  ...
  - **Phase 6 완료 (2026-09-01)** — store tearing(D6, DC-5). 빌드 시작/커밋 직전 버전 비교,
    ...
    돌연변이 5종 검증.
- next: **Phase 7 — T2 진입 게이트 (RC-7)**. 코어 변경 없이 측정과 판정만 한다.
  - **하네스는 준비됐다 — `pnpm bench:units`.** 측정 방법과 설계 근거는 §Phase 7에 있다.
    남은 것은 형이 브라우저에서 돌리고 표를 붙인 뒤 7-3을 판정하는 것.
  - 예비 측정(Phase 3 중, jsdom + 인위적 지연)은 §Phase 7에 따로 있다. 그것은 T1의
    **이득 구간**을 본 것이고 7-3의 답이 아니다 — 실제 렌더 비용을 재지 않는다.
