...
#### 이것이 Phase 9/10에 남기는 것

- **D-신규: `wDomToDom`을 커밋에서 빌드로 옮긴다.** 생성 경로의 중단 가능 비중을
  17%에서 ~75%로 올리는 유일하고 가장 큰 레버다. 폐기 시 만들어둔 노드는 버려지지만
  GC가 처리하며, 부수효과가 없으므로 폐기 안전성 논증도 필요 없다.
- RC-10 판정은 그 이후에 해야 의미가 있다. 지금 상태로 E-4를 돌리면 생성 경로에서
  떨어지고, 그것은 파이버의 한계가 아니라 아직 안 옮긴 작업의 한계다.
...
- next: **`wDomToDom`을 빌드로 옮기기 → Phase 9 — alternate + 폐기/롤백 (D8, D9)**.
  - **순서를 바꿀 것.** 생성 경로의 중단 가능 비중이 17%에 머무는 한 RC-10 판정이
    무의미하다. §Phase 8 "빌드/커밋 비중"의 결론대로 `wDomToDom`을 먼저 옮기고,
    그 다음에 폐기/롤백을 한다.
  - 옮긴 뒤 `pnpm bench:units`를 다시 돌려 표를 갱신한다 (컴포넌트 행 시나리오 포함).
