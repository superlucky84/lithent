...
> 다만 생성 커밋의 71%는 `wDomToDom`, 즉 **문서에 붙지 않은 노드를 만드는 순수 할당**이라
> 빌드 단계로 옮길 수 있다. 옮기면 생성 경로도 ~75%가 중단 가능해진다.
> **RC-10 판정은 그 이후에 해야 의미가 있다.**
...
| **RC-10** | 단일 컴포넌트의 무거운 렌더(10k행) 중 입력이 차단되지 않는다 | 수동 E-4 — **T2의 존재 이유이자 "concurrent" 명명 근거** (§2.1). ⚠ Phase 8 실측: 중단 가능한 빌드 비중이 갱신 48% / 생성 17%다. 생성은 `wDomToDom`을 빌드로 옮기면 ~75%가 된다 (IMPLEMENT §Phase 8) |
