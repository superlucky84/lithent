...
export const Example8Ko = mount(r => {
  ...
  const loadPreset = (preset: keyof typeof presetsKo) => {
    character.v = { ...presetsKo[preset] };
  };

  const randomize = () => {
    character.v = {
      face: optionsKo.face[Math.floor(Math.random() * optionsKo.face.length)]
        .value,
      hair: optionsKo.hair[Math.floor(Math.random() * optionsKo.hair.length)]
        .value,
      eyes: optionsKo.eyes[Math.floor(Math.random() * optionsKo.eyes.length)]
        .value,
      outfit:
        optionsKo.outfit[Math.floor(Math.random() * optionsKo.outfit.length)]
          .value,
      accessory:
        optionsKo.accessory[
          Math.floor(Math.random() * optionsKo.accessory.length)
        ].value,
      background:
        optionsKo.background[
          Math.floor(Math.random() * optionsKo.background.length)
        ].value,
    };
  };
      ...
      <div class="w-full max-w-4xl mx-auto">
        <div class="mb-4">
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400">
            Select를 사용해서 나만의 이모지 캐릭터를 만들어보세요.
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="order-2 md:order-1">
            ...
            <div class="mt-3 text-xs text-gray-500 dark:text-gray-400 text-center">
              💡 Select 변경 시 캐릭터 프리뷰가 실시간으로 업데이트됩니다.
            </div>
          </div>
