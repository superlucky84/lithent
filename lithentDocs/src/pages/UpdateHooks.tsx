...
// 2. 업데이트 후: DOM updated with count 1`}
    ...
    <p class="text-sm md:text-base text-gray-700 dark:text-gray-300 leading-relaxed mb-6">
      ...
      를 전달하면, 지정한 값이 변경되었을 때만 실행됩니다. 이는 불필요한
      실행을 방지하여 성능을 최적화합니다.
    </p>
...
const UserProfile = mount((renew) => {
  ...
  return () => (
    ...
    <p class="text-sm md:text-base text-gray-700 dark:text-gray-300 leading-relaxed mb-6">
      ...
      에 실행됩니다. 이는 업데이트된 DOM에 접근하거나 외부 라이브러리를
      동기화할 때 유용합니다.
    </p>
      ...
      <ul class="space-y-3 text-sm md:text-base text-gray-700 dark:text-gray-300">
        <li class="flex items-start">
          ...
          <div>
            ...
            : 컴포넌트가 <strong>처음 마운트될 때 단 한 번</strong> 실행.
            초기화 작업에 적합.
          </div>
        </li>
        <li class="flex items-start">
    ...
    <div class="border-l-4 border-[#42b883] bg-gradient-to-r from-[#42b883]/5 to-transparent dark:from-[#42b883]/10 dark:to-transparent p-6 mb-6 rounded-r">
      <ol class="space-y-3 text-sm md:text-base text-gray-700 dark:text-gray-300">
        <li class="flex items-start">
          ...
          <span>
            마운터 실행 시 updateCallback 호출로 콜백 함수 등록
          </span>
        </li>
        <li class="flex items-start">
          <span class="font-semibold text-[#42b883] mr-3 flex-shrink-0">
        ...
        <li class="flex items-start">
          ...
          <span>
            등록된 updateCallback들의 dependencies 확인
          </span>
        </li>
        <li class="flex items-start">
          <span class="font-semibold text-[#42b883] mr-3 flex-shrink-0">
        ...
        <li class="flex items-start">
          ...
          <span>
            effectAction이 반환하는 함수를 큐(upCB)에 저장
          </span>
        </li>
        <li class="flex items-start">
          <span class="font-semibold text-[#42b883] mr-3 flex-shrink-0">
        ...
        <li class="flex items-start">
          ...
          <span>
            큐에 저장된 반환 함수들을 순서대로 실행 (DOM 업데이트 후)
          </span>
        </li>
      </ol>
    </div>
...
// 6. "B. DOM 업데이트 후 실행" 출력`}
    ...
    <div class="border-l-4 border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20 p-4 mb-6 rounded-r">
      <p class="text-sm md:text-base text-yellow-800 dark:text-yellow-200 leading-relaxed">
        <span class="font-medium">⚠️ 두 단계 실행 이해하기:</span> updateCallback
        자체는 dependencies 변경 시 즉시 실행되고, 반환 함수는 DOM 업데이트
        후 실행됩니다. 이 차이를 정확히 이해해야 합니다.
        <br />
        <br />
        <span class="font-medium">⚠️ dependencies는 함수로 전달:</span>{' '}
        dependencies는 배열이 아닌{' '}
        <strong>배열을 반환하는 함수</strong>로 전달해야 합니다.{' '}
        ...
        마운트 시점에도 실행됩니다. 마운트 이후 업데이트만 감지하려면
        별도의 플래그를 사용하세요.
      </p>
    </div>
